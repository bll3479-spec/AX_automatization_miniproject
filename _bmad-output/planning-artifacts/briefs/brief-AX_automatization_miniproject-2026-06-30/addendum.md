...
브리프 본문의 "The Problem"에서 요약한 3가지 사례의 원본 데이터와 수동 점수 계산. 다운스트림(PRD/구현) 작업 시 정확한 키워드/발신자 후보를 정할 때 참조.
