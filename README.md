...
이메일을 읽어 **오늘의 소식(뉴스레터) / 공고(채용) / 업무 / 영수증(결제) / 보안 / 프로모션 / 기타** 7개 카테고리로 자동 분류하고 태깅해주는 로컬 웹앱입니다. 첫 화면에는 가장 자주 보는 **오늘의 소식 / 공고 / 업무 / 영수증** 4개를 큰 섹션(2x2)으로 강조하고, 나머지(보안/프로모션/기타)는 하단에 작은 요약 리스트로 보여줍니다.
...
```
SECURITY → PAYMENT → WORK → ANNOUNCEMENT → PROMOTION → NEWSLETTER → OTHER(→ LLM 보조, 설정 시)
```
...
첫 화면(전체 탭)에서는 **오늘의 소식 / 공고 / 업무 / 영수증**이 큰 섹션으로, 나머지(보안/프로모션/기타)는 하단 작은 요약 리스트로 표시됩니다.
